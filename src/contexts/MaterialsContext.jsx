...
const mergePurchasedDefaults = (savedList, defaultList) => {
  const curatedDefaults = defaultList.filter(item => (item.id || 0) >= 74);
  const existingKeys = new Set(savedList.map(item => `${item.name}__${item.date || ''}`));
  const toAppend = curatedDefaults.filter(item => !existingKeys.has(`${item.name}__${item.date || ''}`));
  return [...savedList, ...toAppend];
};
...
export const MaterialsProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (savedMaterials) {
      // ✅ Kayıtlı veri varsa ONU kullan; eksik yeni kurumsal alım kayıtlarını birleştir
      const parsedMaterials = JSON.parse(savedMaterials);
      const mergedMaterials = mergePurchasedDefaults(parsedMaterials, initialInventory);
      setMaterials(mergedMaterials);
      if (mergedMaterials.length !== parsedMaterials.length) {
        localStorage.setItem('chlorellaMaterials', JSON.stringify(mergedMaterials));
      }
    } else {
  ...
  const addMaterial = (material) => {
    ...
    const newMaterial = {
      ...
      id: Date.now(),
      totalCost,
      // ✅ YENİ ALANLAR - Fatura ve KDV bilgileri
      invoiceTotal: material.invoiceTotal || 0,
      shippingCost: material.shippingCost || 0,
      kdvRate: material.kdvRate || 20,
      kdvIncluded: material.kdvIncluded !== false, // Default true
      useInvoiceMode: material.useInvoiceMode || false
    };
  ...
  const updateMaterial = (id, updates) => {
    const updated = materials.map(m => {
      if (m.id === id) {
        ...
        // ✅ FATURA MODU: Fatura tutarından hesapla
        if (newMaterial.useInvoiceMode && newMaterial.invoiceTotal && newMaterial.quantity > 0) {
          // Fatura modda birim fiyat otomatik hesaplanır
          const invoiceBase = newMaterial.kdvIncluded 
            ? newMaterial.invoiceTotal / (1 + newMaterial.kdvRate / 100)
            : newMaterial.invoiceTotal;
          
          newMaterial.unitPrice = (invoiceBase + (newMaterial.shippingCost || 0)) / newMaterial.quantity;
          newMaterial.totalCost = newMaterial.unitPrice * newMaterial.quantity;
        } 
        // ✅ BİRİM FİYAT MODU: Standart hesaplama
        else {
